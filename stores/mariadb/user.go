...
func (u User) FindAll(settings ...Option) ([]models.User, error) {
	...
	stmt, err = db.PrepareContext(ctx, fmt.Sprintf(`SELECT %s FROM %s ORDER BY user_id DESC LIMIT ?,?`, u.QueryColumn, u.TableName))
	if err != nil {
		return mUsers, err
	}
	defer stmt.Close()
...
func (u User) GetTotal() (int64, error) {
	...
	stmt, err = db.PrepareContext(ctx, fmt.Sprintf(`SELECT COUNT(user_id) FROM %s`, u.TableName))
	if err != nil {
		return 0, err
	}
	defer stmt.Close()
